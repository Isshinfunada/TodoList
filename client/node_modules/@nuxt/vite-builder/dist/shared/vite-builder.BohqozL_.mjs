...
import { dirname, normalize, isAbsolute, resolve as resolve$1, relative, join } from 'pathe';
import { useNuxt, useNitro, logger, resolvePath, tryResolveModule, requireModule, isIgnored, addVitePlugin } from '@nuxt/kit';
import replace from '@rollup/plugin-replace';
import { resolve, interopDefault, findStaticImports, parseStaticImport, sanitizeFilePath } from 'mlly';
      ...
      load: {
        ...
        handler(id) {
          if (id.startsWith(PREFIX$1)) {
            return `import { publicAssetsURL } from '#internal/nuxt/paths';export default publicAssetsURL(${JSON.stringify(decodeURIComponent(id.slice(PREFIX$1.length)))})`;
          }
        }
      },
      resolveId: {
        ...
        handler(id) {
          ...
          if (resolveFromPublicAssets(id)) {
            return PREFIX$1 + encodeURIComponent(id);
          }
        }
      },
...
async function buildClient(ctx) {
  ...
  const clientConfig = vite.mergeConfig(ctx.config, vite.mergeConfig({
    ...
    cacheDir: resolve$1(ctx.nuxt.options.rootDir, ctx.config.cacheDir ?? "node_modules/.cache/vite", "client"),
    build: {
...
async function writeManifest(ctx, css = []) {
  ...
  if (ctx.config.build?.cssCodeSplit === false) {
    for (const key in clientManifest) {
      ...
      if (val.file?.endsWith(".css")) {
        const key2 = relative(ctx.config.root, ctx.entry);
        (_a = clientManifest[key2]).css || (_a.css = []);
        clientManifest[key2].css.push(val.file);
        break;
      }
    }
  }
  ...
  await writeFile(resolve$1(serverDist, "client.manifest.json"), stringifiedManifest, "utf8");
  await writeFile(resolve$1(serverDist, "client.manifest.mjs"), "export default " + stringifiedManifest, "utf8");
  if (!ctx.nuxt.options.dev) {
...
async function buildServer(ctx) {
  ...
  const serverConfig = vite.mergeConfig(ctx.config, vite.mergeConfig({
    ...
    cacheDir: resolve$1(ctx.nuxt.options.rootDir, ctx.config.cacheDir ?? "node_modules/.cache/vite", "server"),
    build: {
  ...
  if (!ctx.nuxt.options.dev) {
    const nitroDependencies = await tryResolveModule("nitropack/package.json", ctx.nuxt.options.modulesDir).then((r) => import(r)).then((r) => r.dependencies ? Object.keys(r.dependencies) : []).catch(() => []);
  ...
  return {
    ...
    renderChunk(_code, chunk) {
      ...
      for (const moduleId of [chunk.facadeModuleId, ...chunk.moduleIds].filter(Boolean)) {
        if (options.mode === "client") {
          const moduleMap = (_c = options.clientCSSMap)[moduleId] || (_c[moduleId] = /* @__PURE__ */ new Set());
          if (isCSS(moduleId)) {
            if (isVue(moduleId)) {
              moduleMap.add(moduleId);
              const parent = moduleId.replace(/\?.+$/, "");
              const parentMap = (_d = options.clientCSSMap)[parent] || (_d[parent] = /* @__PURE__ */ new Set());
              parentMap.add(moduleId);
            }
            if (isEntry && chunk.facadeModuleId) {
              const facadeMap = (_e = options.clientCSSMap)[_f = chunk.facadeModuleId] || (_e[_f] = /* @__PURE__ */ new Set());
    ...
    async transform(code, id) {
      ...
      if (options.mode === "client") {
        if (id === options.entry && (options.shouldInline === true || typeof options.shouldInline === "function" && options.shouldInline(id))) {
          const s = new MagicString(code);
          const idClientCSSMap = (_a = options.clientCSSMap)[id] || (_a[id] = /* @__PURE__ */ new Set());
...
`);
      ...
      const idMap = cssMap[relativeId] || (cssMap[relativeId] = { files: [] });
