// node_modules/.pnpm/tsup@8.2.2_jiti@1.21.6_typescript@5.5.4/node_modules/tsup/assets/esm_shims.js
...
function setSourceContent(map, source, content) {
  const { _sources: sources2, _sourcesContent: sourcesContent } = cast3(map);
  const index = put(sources2, source);
  sourcesContent[index] = content;
}
function setIgnore(map, source, ignore = true) {
  const { _sources: sources2, _sourcesContent: sourcesContent, _ignoreList: ignoreList } = cast3(map);
  const index = put(sources2, source);
...
function toDecodedMap(map) {
  const { _mappings: mappings, _sources: sources2, _sourcesContent: sourcesContent, _names: names, _ignoreList: ignoreList } = cast3(map);
...
function addSegmentInternal(skipable, map, genLine, genColumn, source, sourceLine, sourceColumn, name, content) {
  const { _mappings: mappings, _sources: sources2, _sourcesContent: sourcesContent, _names: names } = cast3(map);
...
function Source(map, sources2, source, content, ignore) {
  return {
    map,
    sources: sources2,
    source,
...
function MapSource(map, sources2) {
  return Source(map, sources2, "", null, false);
}
...
function createBuildContext2(options, compilation) {
  return {
    parse(code, opts = {}) {
    ...
    addWatchFile(id) {
      options.addWatchFile(resolve3(process.cwd(), id));
    },
    emitFile(emittedFile) {
      const outFileName = emittedFile.fileName || emittedFile.name;
      if (emittedFile.source && outFileName) {
        const { sources: sources2 } = compilation.compiler.webpack;
        ...
        for (const rawPlugin of rawPlugins) {
          ...
          if (plugin.resolveId) {
            compiler.hooks.compilation.tap(plugin.name, (compilation, { normalModuleFactory }) => {
              normalModuleFactory.hooks.resolve.tapPromise(plugin.name, async (resolveData) => {
                ...
                const context = createBuildContext2(contextOptionsFromCompilation(compilation), compilation);
                let error;
                const pluginContext = {
                  error(msg) {
                    if (error == null)
          ...
          if (plugin.buildEnd) {
            compiler.hooks.emit.tapPromise(plugin.name, async (compilation) => {
              await plugin.buildEnd.call(createBuildContext2(contextOptionsFromCompilation(compilation), compilation));
            });
          }
          if (plugin.writeBundle) {
...
function contextOptionsFromCompilation2(compilation) {
  return {
    addWatchFile(file) {
    ...
    return {
      apply(compiler) {
        ...
        for (const rawPlugin of rawPlugins) {
          ...
          if (plugin.watchChange || plugin.buildStart) {
            compiler.hooks.make.tapPromise(plugin.name, async (compilation) => {
              const context = createBuildContext3(contextOptionsFromCompilation2(compilation), compilation);
              if (plugin.watchChange && (compiler.modifiedFiles || compiler.removedFiles)) {
                const promises = [];
                if (compiler.modifiedFiles) {
                  compiler.modifiedFiles.forEach(
          ...
          if (plugin.buildEnd) {
            compiler.hooks.emit.tapPromise(plugin.name, async (compilation) => {
              await plugin.buildEnd.call(createBuildContext3(contextOptionsFromCompilation2(compilation), compilation));
            });
          }
          if (plugin.writeBundle) {
            compiler.hooks.afterEmit.tapPromise(plugin.name, async () => {
