...
const componentsTypeTemplate = {
  ...
  getContents: ({ app, nuxt }) => {
    ...
    const componentTypes = app.components.filter((c) => !c.island).map((c) => {
      const type = `typeof ${genDynamicImport(isAbsolute(c.filePath) ? relative(buildDir, c.filePath).replace(/\b\.(?!vue)\w+$/g, "") : c.filePath.replace(/\b\.(?!vue)\w+$/g, ""), { wrapper: false })}['${c.export}']`;
      return [
        c.pascalName,
...
const loaderPlugin = createUnplugin((options) => {
  ...
  return {
    ...
    transform(code) {
      ...
      const s = new MagicString(code);
      s.replace(/(?<=[ (])_?resolveComponent\(\s*["'](lazy-|Lazy)?([^'"]*)["'][^)]*\)/g, (full, lazy, name) => {
...
const islandsTransform = createUnplugin((options, meta) => {
  ...
  return {
    ...
    transformInclude(id) {
      ...
      if (options.selectiveClient === "deep") {
        return true;
      }
      const components = options.getComponents();
      const islands = components.filter(
...
function createTransformPlugin(nuxt, getComponents, mode) {
  ...
  function getComponentsImports() {
    ...
    return components.flatMap((c) => {
      const withMode = (mode3) => mode3 ? `${c.filePath}${c.filePath.includes("?") ? "&" : "?"}nuxt_component=${mode3}&nuxt_component_name=${c.pascalName}&nuxt_component_export=${c.export || "default"}` : c.filePath;
...
const template = () => {
  return '<svg class="nuxt-spa-loading" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 37 25" fill="none" width="80"><path d="M24.236 22.006h10.742L25.563 5.822l-8.979 14.31a4 4 0 0 1-3.388 1.874H2.978l11.631-20 5.897 10.567"/></svg><style>.nuxt-spa-loading {\n    position: fixed;\n    top: 50%;\n    left: 50%;\n    transform: translate(-50%, -50%);\n  }\n\n  .nuxt-spa-loading>path {\n    fill: none;\n    stroke: #00DC82;\n    stroke-width: 4px;\n    stroke-linecap: round;\n    stroke-linejoin: round;\n    /* Stroke-dasharray property */\n    stroke-dasharray: 128;\n    stroke-dashoffset: 128;\n    animation: nuxt-spa-loading-move 3s linear infinite;\n  }\n\n  @keyframes nuxt-spa-loading-move {\n    100% {\n      stroke-dashoffset: -128;\n    }\n  }</style>';
};
...
async function initNitro(nuxt) {
  ...
  for (const _alias in aliases) {
    ...
    if (stats?.isDirectory()) {
      ...
      tsConfig.compilerOptions.paths[alias] = [absolutePath.replace(/\b\.\w+$/g, "")];
    }
  }
  ...
  const cacheDriverPath = await resolvePath$1(join(distDir, "core/runtime/nitro/cache-driver"));
...
async function initNuxt(nuxt) {
    ...
    async function promptAndUpdate() {
      ...
      try {
        const res = await updateConfig({
          ...
          onUpdate(config2) {
            config2.compatibilityDate = todaysDate;
          }
        });
        if (res?.configFile) {
  ...
  const dependencies = /* @__PURE__ */ new Set([...Object.keys(packageJSON.dependencies || {}), ...Object.keys(packageJSON.devDependencies || {})]);
  const paths = Object.fromEntries(await Promise.all(coreTypePackages.map(async (pkg) => {
    ...
    if (dependencies.has(_pkg) && !(_pkg in nightlies)) {
      return [];
    }
  ...
  for (const dir of pathsToWatch) {
    ...
    watcher.on("all", (event, path) => {
      ...
      if (event === "addDir" && path !== dir && !ignoredDirs.has(path) && !pathsToWatch.includes(path) && !(path in watchers) && !isIgnored(path)) {
        ...
        watchers[path].on("all", (event2, p) => nuxt.callHook("builder:watch", event2, nuxt.options.experimental.relativeWatchPaths ? normalize(relative(nuxt.options.srcDir, p)) : normalize(p)));
