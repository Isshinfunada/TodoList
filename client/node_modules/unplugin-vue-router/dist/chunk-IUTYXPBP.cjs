...
var _chunkPSZLSTUIcjs = require('./chunk-PSZLSTUI.cjs');
...
var _TreeNodeValueBase = (_class = class {
  ...
  get path() {
    ...
    return (!parentPath || parentPath === "/") && pathSegment === "" ? "/" : _chunkPSZLSTUIcjs.joinPath.call(void 0, parentPath || "", pathSegment);
  }
  toString() {
  ...
  get overrides() {
    return [...this._overrides.entries()].sort(
      ...
      return _chunkPSZLSTUIcjs.mergeRouteRecordOverride.call(void 0, acc, routeBlock);
    }, {});
  }
  ...
  mergeOverride(filePath, routeBlock) {
    ...
    this._overrides.set(
      filePath,
      _chunkPSZLSTUIcjs.mergeRouteRecordOverride.call(void 0, existing, routeBlock)
    );
  }
  /**
...
var TreeNodeValueParam = (_class3 = class extends _TreeNodeValueBase {
  
  __init4() {this._type = 1} /* param */
  constructor(rawSegment, parent, params, pathSegment, subSegments) {
    super(rawSegment, parent, pathSegment, subSegments);_class3.prototype.__init4.call(this);;
    this.params = params;
  }
}, _class3);
...
// src/core/tree.ts
var TreeNode = (_class4 = class _TreeNode {
  /**
   * value of the node
   */
   ...
   */
  __init5() {this.children = /* @__PURE__ */ new Map()}
  /**
   * Parent node.
   */
   ...
   */
  __init6() {this.hasDefinePage = false}
  /**
   ...
   * @param parent
   */
  constructor(options, pathSegment, parent) {;_class4.prototype.__init5.call(this);_class4.prototype.__init6.call(this);
...
var PrefixTree = (_class5 = class extends TreeNode {
  __init7() {this.map = /* @__PURE__ */ new Map()}
  constructor(options) {
    super(options, "");_class5.prototype.__init7.call(this);;
  }
  insert(path, filePath) {
...
function getRouteBlock(path, content, options) {
  ...
  if (result) {
    if (result.path != null && !result.path.startsWith("/")) {
      _chunkPSZLSTUIcjs.warn.call(void 0, `Overridden path must start with "/". Found in "${path}".`);
    }
  }
  return result;
}
function parseCustomBlock(block, filePath, options) {
  ...
  if (lang === "json5") {
    try {
      ...
      _chunkPSZLSTUIcjs.warn.call(void 0, 
...
var RoutesFolderWatcher = class {
  ...
  constructor(folderOptions) {
    ...
    this.watcher = _chokidar2.default.watch(folderOptions.pattern, {
      cwd: this.src,
      ignoreInitial: true,
  ...
  on(event, handler) {
    this.watcher.on(event, (filePath) => {
      ...
      handler({
        filePath,
        routePath: _chunkPSZLSTUIcjs.asRoutePath.call(void 0, 
          {
            src: this.src,
            path: this.path,
...
function resolveFolderOptions(globalOptions, folderOptions) {
  ...
  return {
    ...
    pattern: _chunkPSZLSTUIcjs.appendExtensionListToPattern.call(void 0, 
      filePatterns,
...
function definePageTransform({
  ...
  const definePageNodes = (_optionalChain([setupAst, 'optionalAccess', _10 => _10.body]) || []).map((node) => {
  ...
  if (isExtractingDefinePage) {
    ...
    const scriptBindings = _optionalChain([setupAst, 'optionalAccess', _11 => _11.body]) ? getIdentifiers(setupAst.body) : [];
    ...
    _astwalkerscope.walkAST.call(void 0, routeRecord, {
      enter(node) {
        if (_optionalChain([this, 'access', _12 => _12.parent, 'optionalAccess', _13 => _13.type]) === "ObjectProperty" && this.parent.key === node && // still track computed keys [a + b]: 1
        !this.parent.computed && node.type === "Identifier") {
          this.skip();
        } else if (
          // filter out things like 'log' in console.log
          _optionalChain([this, 'access', _14 => _14.parent, 'optionalAccess', _15 => _15.type]) === "MemberExpression" && this.parent.property === node && !this.parent.computed && node.type === "Identifier"
        ) {
          this.skip();
...
function extractDefinePageNameAndPath(sfcCode, id) {
  ...
  const setupAst = getSetupAst();
  const definePageNodes = (_nullishCoalesce(_optionalChain([setupAst, 'optionalAccess', _16 => _16.body]), () => ( []))).map((node) => {
    if (node.type === "ExpressionStatement") node = node.expression;
    return _common.isCallOf.call(void 0, node, MACRO_DEFINE_PAGE) ? node : null;
  }).filter((node) => !!node);
  if (!definePageNodes.length) {
    ...
    if (prop.type === "ObjectProperty" && prop.key.type === "Identifier") {
      if (prop.key.name === "name") {
        if (prop.value.type !== "StringLiteral") {
          _chunkPSZLSTUIcjs.warn.call(void 0, `route name must be a string literal. Found in "${id}".`);
        } else {
          routeInfo.name = prop.value.value;
        }
      } else if (prop.key.name === "path") {
        if (prop.value.type !== "StringLiteral") {
          _chunkPSZLSTUIcjs.warn.call(void 0, `route path must be a string literal. Found in "${id}".`);
        } else {
...
function createRoutesContext(options) {
  ...
  async function scanPages(startWatchers = true) {
        ...
        return _fastglob2.default.call(void 0, folder.pattern, {
          ...
          (files) => Promise.all(
            files.map((file) => _pathe.resolve.call(void 0, folder.src, file)).map(
              (file) => addPage({
                routePath: _chunkPSZLSTUIcjs.asRoutePath.call(void 0, folder, file),
                filePath: file
              })
            )
          )
    ...
    for (const route of editableRoutes) {
      await _optionalChain([options, 'access', _17 => _17.extendRoute, 'optionalCall', _18 => _18(route)]);
    }
  ...
  async function addPage({ filePath, routePath }, triggerExtendRoute = false) {
    ...
    if (triggerExtendRoute) {
      await _optionalChain([options, 'access', _19 => _19.extendRoute, 'optionalCall', _20 => _20(new EditableTreeNode(node))]);
    }
    _optionalChain([server, 'optionalAccess', _21 => _21.updateRoutes, 'call', _22 => _22()]);
  }
  async function updatePage({ filePath, routePath }) {
    ...
    await writeRouteInfoToNode(node, filePath);
    await _optionalChain([options, 'access', _23 => _23.extendRoute, 'optionalCall', _24 => _24(new EditableTreeNode(node))]);
  }
  function removePage({ filePath, routePath }) {
    logger.log(`remove "${routePath}" for "${filePath}"`);
    routeTree.removeChild(filePath);
    _optionalChain([server, 'optionalAccess', _25 => _25.updateRoutes, 'call', _26 => _26()]);
  }
  ...
  function generateRoutes() {
    const importsMap = new (0, _chunkPSZLSTUIcjs.ImportsMap)();
...
`;
  ...
  async function _writeConfigFiles() {
    ...
    _chunkPSZLSTUIcjs.logTree.call(void 0, routeTree, logger.log);
    if (dts) {
  ...
  const writeConfigFiles = _chunkPSZLSTUIcjs.throttle.call(void 0, _writeConfigFiles, 500, 100);
...
var src_default = _unplugin.createUnplugin.call(void 0, (opt = {}, _meta) => {
  const options = _chunkPSZLSTUIcjs.resolveOptions.call(void 0, opt);
  ...
  const pageFilePattern = _chunkPSZLSTUIcjs.appendExtensionListToPattern.call(void 0, 
    options.filePatterns,
    _chunkPSZLSTUIcjs.mergeAllExtensions.call(void 0, options)
  );
  const filterPageComponents = _pluginutils.createFilter.call(void 0, 
    [
      ...
      handleHotUpdate: {
        ...
        handler({ server, file, modules }) {
          ...
          const definePageModule = Array.from(moduleList || []).find((mod) => {
            return _optionalChain([mod, 'optionalAccess', _27 => _27.id]) && MACRO_DEFINE_PAGE_QUERY.test(mod.id);
          });
          if (definePageModule) {
...
exports.createTreeNodeValue = createTreeNodeValue; exports.EditableTreeNode = EditableTreeNode; exports.createRoutesContext = createRoutesContext; exports.src_default = src_default; exports.VueRouterAutoImports = VueRouterAutoImports;
